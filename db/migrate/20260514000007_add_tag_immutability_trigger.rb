class AddTagImmutabilityTrigger < ActiveRecord::Migration[8.0]
  def up
    execute <<~SQL
  ...
  def down
    execute <<~SQL
      ...
      DROP FUNCTION IF EXISTS prevent_task_tag_delete();
      DROP FUNCTION IF EXISTS prevent_tag_delete();
      DROP FUNCTION IF EXISTS prevent_system_tag_mutation();
    SQL
  end
end
